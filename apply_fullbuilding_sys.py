...
def make_collection_instances_real(collection):
    ...
    print(f"[make_collection_instances_real] ✅ {len(new_objects)} objetos creados en '{collection.name}'")
    return new_objects

def expand_volume(container, expansion):
    bbox_corners = [container.matrix_world @ Vector(corner) for corner in container.bound_box]
...
def organize_objects():
    collections = {"00_Buildings": "Buildings_Exp","01_Facades": "Facades_Exp","02_Numbers": "Numbers_Exp"}
...
def detect_walls_inside_buildings():
    ...
    if not buildings_collection or not facades_collection:
        return
...
# REGISTRO DE CLASES
# ===================================================
classes = (OBJECT_OT_apply_fullbuilding_sys,)
