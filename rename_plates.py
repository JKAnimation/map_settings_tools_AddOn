...
def clean_duplicates(collection, tolerance=0.01):
    ...
    for obj in collection.objects:
        ...
        frozen_position = position.copy()
        frozen_position.freeze()
...
def calculate_midpoint(objects):
    sum_position = Vector((0, 0, 0))
    for obj in objects:
        sum_position += obj.location
    midpoint = sum_position / len(objects) if objects else Vector((0, 0, 0))
    midpoint.z = 0  # Ignorar la coordenada Z
    return midpoint

def get_angle_from_center(obj, center, reference):
    vec = obj.location - center
    vec.z = 0  # Ignorar la coordenada Z
    ref_vec = reference - center
    ref_vec.z = 0
    angle = math.atan2(vec.y, vec.x) - math.atan2(ref_vec.y, ref_vec.x)
    return angle % (2 * math.pi)

def sort_letters_ccw(letters, center, reference):
    return sorted(letters, key=lambda l: get_angle_from_center(l, center, reference))
    ...
    def execute(self, context):
        try:
            ...
            if not buildings_collection or not facades_collection or not plates_collection or not numbers_collection or not letters_collection:
                self.report({'ERROR'}, "Faltan colecciones necesarias")
                return {'CANCELLED'}
            ...
            for building in buildings_collection.objects:
                ...
                facades_inside = [entry for entry in facades_collection.objects if is_point_inside_volume(building, entry.location)]
                if not facades_inside:
                    self.report({'WARNING'}, f"No se encontraron entradas dentro del edificio: {building.name}")
                    continue
                
                plates_inside = [plate for entry in facades_inside for plate in plates_collection.objects if is_point_inside_volume(entry, plate.location)]
                if not plates_inside:
                    self.report({'WARNING'}, f"No se encontraron placas dentro de las entradas en el edificio: {building.name}")
                    continue
                
                letters_inside = [letter for plate in plates_inside for letter in letters_collection.objects if is_point_inside_volume(plate, letter.location)]
                ...
                center = calculate_midpoint(letters_inside)
                letter_A = next((l for l in letters_inside if l.data.body.startswith("A")), None)
                
                if not letter_A:
                    continue
                
                sorted_letters = sort_letters_ccw(letters_inside, center, letter_A.location)
            ...
            self.report({'INFO'}, "Placas renombradas correctamente")
            return {'FINISHED'}
